...
class CyclomaticStrictMetric:
    def __init__(self, entity_longname='Project'):
        ...
        if entity_longname != 'Project':
            ...
            self.method = entity
            listener = CyclomaticStrictMetricListener(method_entity=self.method)
        else:
            self.files = get_all_files()
            listener = CyclomaticStrictMetricListener()
        for file_content in self.files:
            parse_tree = AntlrHandler.Parse(file_content)
            AntlrHandler.Walk(listener, parse_tree)
        print(listener.cyclomatic_strict_metric)
...
class CyclomaticStrictMetricListener(JavaParserLabeledListener):
    ...
    @property
    def cyclomatic_strict_metric(self):
        return self.count_cyclomatic_strict_metric
    ...
    def enterSwitchLabel(self, ctx: JavaParserLabeled.SwitchLabelContext):
        if 'case' in ctx.getText():
            if self.method is not None:
                ...
                self.count_cyclomatic_strict_metric += 1
        # print('enter case clause', ctx.getText())
