...
class LineOfCode(JavaParserLabeledListener):
    ...
    def enterClassDeclaration(self, ctx:JavaParserLabeled.ClassDeclarationContext):
        ...
        self.method_countLineComment.append(('class '+ctx.IDENTIFIER().getText(), self.countLineCodeComment(tokens[1])))
        self.method_countLineCode.append(('class '+ctx.IDENTIFIER().getText(), len(tokens[0])))

    def enterMethodDeclaration(self, ctx:JavaParserLabeled.MethodDeclarationContext):
        tokens=self.find(ctx.start,ctx.stop)
        self.method_countLineDecl.append(('method '+ctx.IDENTIFIER().getText(), self.countLineCodeDecl(tokens[0])))
        self.method_countLineComment.append(('method '+ctx.IDENTIFIER().getText(), self.countLineCodeComment(tokens[1])))
        self.method_countLineCode.append(('method '+ctx.IDENTIFIER().getText(),len(tokens[0])))

    def enterMethodBody(self, ctx:JavaParserLabeled.MethodBodyContext):
        tokens=self.find(ctx.start,ctx.stop)
        self.method_countLineExec.append(('method '+ctx.parentCtx.IDENTIFIER().getText(),self.countLineCodeExec(tokens[0])))
    ...
    def exitClassDeclaration(self, ctx:JavaParserLabeled.ClassDeclarationContext):
        ...
        self.method_countLineDecl=[]
        self.class_countLineComment[ctx.IDENTIFIER().getText()] = self.method_countLineComment
        self.method_countLineComment=[]
    ...
    def enterInterfaceDeclaration(self, ctx:JavaParserLabeled.InterfaceDeclarationContext):
        tokens=self.find(ctx.start,ctx.stop)
        self.interface_countLineDecl['interface '+ctx.IDENTIFIER().getText()] = self.countLineCodeDecl(tokens[0])
        self.interface_countLineComment['interface '+ctx.IDENTIFIER().getText()] = self.countLineCodeComment(tokens[1])
        self.interface_countLineCode['interface '+ctx.IDENTIFIER().getText()] = len(tokens[0])
    ...
    def countLineCodeDecl(self,tokens):
        ...
        for j in tokens:
            for i in j:
                if (i.type in [31,9,48,27,14,8,5,28,29,20,3,37]) and i.channel==0:
                    #print(i.type)
                    counter+=1
                    break
