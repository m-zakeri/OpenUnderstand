...
class Project():
    ...
    def addCreateRefs(self, ref_dicts, file_ent, file_address):
        for ref_dict in ref_dicts:
            scope = EntityModel.get_or_create(_kind=self.findKindWithKeywords(ref_dict["parent_type"], ref_dict["scopemodifiers"]),
    ...
    def getPackageEntity(self, file_ent, name, longname):
        # package kind id: 72
        ent = EntityModel.get_or_create(_kind=72, _name=name, _parent=file_ent, _longname=longname, _contents="")
        return ent[0]

    def getUnnamedPackageEntity(self, file_ent):
        # unnamed package kind id: 73
        ent = EntityModel.get_or_create(_kind=73, _name="(Unnamed_Package)", _parent=file_ent, _longname="(Unnamed_Package)", _contents="")
        return ent[0]
    ...
    def getClassEntity(self, class_longname, file_address):
        ...
        if not props:  # This class is unknown, unknown class id: 84
            ent = EntityModel.get_or_create(_kind=84, _name=class_longname.split(".")[-1], _longname=class_longname, _contents="")
        else:
            if len(props["modifiers"]) == 0:
                props["modifiers"].append("default")
            kind = self.findKindWithKeywords("Class", props["modifiers"])
            ent = EntityModel.get_or_create(_kind=kind, _name=props["name"], _longname=props["longname"],
    ...
    for file_address in java_files:
        ...
        try:
            ...
            for files in listener.create:
                ...
                print(f'2. ref.scope(entity performing reference)\t: "{files["scopelongname"]}", kind: "{" ".join(files["scopemodifiers"]).title()} {files["parent_type"]}"')
                print(f'3. ref.entity(entity being referenced)\t\t: {entity_file_longname}')
                print(f'4. location the reference occurred: "{file_address}", line: {files["line"]}')
                print(n)
                print(f'')
                n = n + 1
        except Exception as e:
