...
from kind_id_mapper import get_name_entity

DB_PATH = "../database/xerces2j.oudb"
...
def add_classes_to_entity(ref_dict, file_path):
    kind_name = get_name_entity(ref_dict["prefixes"])
    ...
    for file_path, file_name in zip(p.file_paths, p.file_names):
        try:
            ...
            for entity_dict in extend_listener.inherited_classes:
                child_entity, parent_entity = add_classes_to_entity(entity_dict, file_path)
